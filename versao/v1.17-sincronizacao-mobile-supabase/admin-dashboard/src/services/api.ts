...
export const checkUserAccess = async (email: string, password: string) => {
    try {
        const { data, error } = await supabase
            ...
            .select('*')
            .eq('email', email)
            .eq('senha', password) // Comparação direta conforme solicitado. Ideal seria hash.
            .single();
...
export const fetchCollaborators = async (): Promise<Collaborator[]> => {
    try {
        ...
        return (data || []).map((row: any) => {
            ...
            // Lógica de Status (Deve ser idêntica ao Mobile)
            // Se tem data de liberação preenchida e válida no campo original, está LIBERADO
            const liberationDateRaw = getFieldDefensive(row, ['liberacao_ecoordin', 'liberacao']);
            const isFullyLiberated = !!liberationDateRaw && liberationDateRaw.toString().trim() !== '' && liberationDateRaw.toString().toLowerCase() !== 'pendente';
