...
import {
  ChevronRight, RotateCcw, Search, Users, Clock, ShieldCheck,
  Zap, Lock, AlertCircle, Timer, LogOut, ListTodo, LogIn as LogInIcon
} from 'lucide-react';
...
const ModernMobileSimulator: React.FC<{
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-0 md:p-4 lg:p-8 font-sans overflow-hidden">
      {/* Device Frame - Only on XL screens (Desktop Preview) */}
      <div className={`relative w-full h-[100dvh] xl:w-[420px] xl:h-[860px] xl:rounded-[4rem] xl:p-3.5 xl:shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)] flex flex-col transition-all duration-700 xl:border-[10px]
      ...
      `}>
        {/* Physical Buttons Simulation - Desktop Only */}
        <div className={`hidden xl:block absolute -left-[14px] top-32 w-[4px] h-12 rounded-l-md border-y border-white/5 shadow-inner transition-colors duration-700 ${isSilver ? 'bg-slate-400' : isGold ? 'bg-[#b48d4a]' : 'bg-slate-800'}`} />
        <div className={`hidden xl:block absolute -left-[14px] top-48 w-[4px] h-16 rounded-l-md border-y border-white/5 shadow-inner transition-colors duration-700 ${isSilver ? 'bg-slate-400' : isGold ? 'bg-[#b48d4a]' : 'bg-slate-800'}`} />
        <div className={`hidden xl:block absolute -left-[14px] top-68 w-[4px] h-16 rounded-l-md border-y border-white/5 shadow-inner transition-colors duration-700 ${isSilver ? 'bg-slate-400' : isGold ? 'bg-[#b48d4a]' : 'bg-slate-800'}`} />
        <div className={`hidden xl:block absolute -right-[14px] top-44 w-[4px] h-24 rounded-r-md border-y border-white/5 shadow-inner transition-colors duration-700 ${isSilver ? 'bg-slate-400' : isGold ? 'bg-[#b48d4a]' : 'bg-slate-800'}`} />

        {/* Camera / Notch Variation - Desktop Only */}
        {isMidnight && (
          <div className="hidden xl:flex absolute top-6 left-1/2 -translate-x-1/2 w-28 h-7 bg-black rounded-full z-[100] items-center justify-center border border-white/5 shadow-xl transition-all duration-700">
        ...
        {(isSilver || isGold) && (
          <div className="hidden xl:flex absolute top-7 left-1/2 -translate-x-1/2 w-4 h-4 bg-black rounded-full z-[100] items-center justify-center border border-white/10 shadow-inner transition-all duration-700">
        ...
        <div className="flex-1 w-full bg-app-bg overflow-hidden xl:rounded-[3.2rem] relative flex flex-col shadow-inner xl:pt-20 transition-all duration-700 font-sans">
          ...
          <div className="hidden xl:flex flex-none h-6 items-center justify-center pb-2 bg-app-bg">
...
const App: React.FC = () => {
  ...
  const stats = {
    total: data.length,
    liberados: data.filter(c => c.status === 'LIBERADO').length,
  ...
  const frameSelector = (
    <div className="hidden xl:flex fixed right-10 top-1/2 -translate-y-1/2 flex-col gap-6 bg-white/80 backdrop-blur-xl p-6 rounded-[2.5rem] border border-slate-200 shadow-2xl shadow-slate-200/50 z-[200]">
              ...
              {activeTab === 'dash' && (
                <motion.div key="dash" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="space-y-6 pt-4 pb-24">
                  <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-2 gap-4">
                    <DashboardCard title="Total Pendentes" value={stats.pendentes} icon={<Clock size={20} />} delay={0.1} />
                    <DashboardCard title="Total Liberados" value={stats.liberados} icon={<ShieldCheck size={20} />} delay={0.2} />
                  </div>
                  ...
                  <div className="space-y-4">
                    {(() => {
                      ...
                      const liberados = statusCounts.find(s => s.name === 'LIBERADO') || { name: 'LIBERADO', count: 0 };
                      const otherStatuses = statusCounts.filter(s => s.name !== 'LIBERADO');
                          ...
                          >
                            <div className="relative z-10 flex justify-between items-center">
                              <div className="flex flex-col gap-1">
                                ...
                                <h3 className="text-2xl font-black tracking-tight font-display">{liberados.name}</h3>
                              </div>
                              <div className="flex flex-col items-end">
                                <span className="text-5xl font-black font-display drop-shadow-lg">{liberados.count}</span>
                ...
                <motion.div key="lead" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-6 pt-4 pb-20">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-1 gap-4">
                    {filteredData
                      ...
                      .sort((a, b) => {
                        if (a.status === 'LIBERADO' && b.status !== 'LIBERADO') return -1;
                        if (a.status !== 'LIBERADO' && b.status === 'LIBERADO') return 1;
                        return 0;
                      })
                      .map((collab, i) => (
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col items-end gap-1.5 min-w-[80px]">
          ...
          <div className={`text-[10px] font-bold px-3 py-1 rounded-full ${collab.status === 'LIBERADO' ? 'bg-success/10 text-success' : 'bg-brand-primary/10 text-brand-primary'}`}>
            {collab.status}
          </div>
        </div>
      </button>
    ...
    <motion.div layout className="card-floating overflow-hidden p-0 border-none mb-4">
      ...
      >
        ...
        <div className="flex items-center gap-4">
          {collaborators.some(c => c.status !== 'LIBERADO') && (
            ...
            <div className="p-3 space-y-2">
              {collaborators.map((collab, i) => (
                <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-white border border-app-border shadow-sm">
                  ...
                  <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${collab.status === 'LIBERADO' ? 'bg-success/10 text-success' : 'bg-brand-primary/10 text-brand-primary'}`}>
                    {collab.status}
                  </span>
                </div>
              ))}
...
const LiberationView: React.FC = () => {
  ...
  const isReleased = (date?: string) => {
    ...
    const str = String(date).trim().toUpperCase();
    return str !== 'PENDENTE' && str !== '' && str.length > 5;
  };
  ...
  const filteredList = allData.filter(item => {
    ...
    if (monthFilter !== 'ALL') {
      ...
      const matchesMonth = (
        monthFilter === 'DEC' ? date.month() === 11 :
          monthFilter === 'JAN' ? date.month() === 0 :
            monthFilter === 'FEB' ? date.month() === 1 :
              monthFilter === 'MAR' ? date.month() === 2 :
                true
      );
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 pt-4 pb-24 px-1">
      ...
      {!result && (
        <div className="bg-white p-5 rounded-[32px] border border-app-border shadow-soft space-y-5">
          {/* Status Card */}
          <div className="bg-[#F4F5F7] p-6 rounded-3xl relative overflow-hidden">
            <div className="flex flex-col gap-4">
              <span className="text-[10px] font-black text-[#5A6270] uppercase tracking-[0.2em] font-display text-center">Status de Liberação</span>
              <div className="flex items-center justify-center gap-6">
                <div className="flex items-center gap-2">
                  <span className="text-3xl font-black text-app-text tabular-nums font-display">{releasedCount}</span>
                  <span className="text-[9px] font-black text-[#00C192] uppercase tracking-widest">Liberados</span>
                </div>
                <div className="w-px h-8 bg-[#D1D5DB]" />
                <div className="flex items-center gap-2">
                  <span className="text-3xl font-black text-[#FF4D4D] tabular-nums font-display">{pendingCount}</span>
                  <span className="text-[9px] font-black text-[#FF4D4D] uppercase tracking-widest">Pendentes</span>
                </div>
              </div>
            </div>
            <div className="absolute top-1/2 -right-2 -translate-y-1/2 w-12 h-12 bg-white/40 rounded-full flex items-center justify-center backdrop-blur-sm">
              <Zap className="text-[#5A6270]/40" size={24} fill="currentColor" />
            </div>
          </div>
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-[#5A6270] uppercase ml-2 tracking-widest font-display opacity-80">Função</label>
              <select
                value={roleFilter}
                onChange={(e) => setRoleFilter(e.target.value)}
                className="w-full bg-[#F4F5F7] border border-transparent rounded-2xl px-5 py-3.5 text-xs font-bold text-app-text outline-none focus:border-brand-primary transition-all appearance-none cursor-pointer"
              >
            ...
            <div className="grid grid-cols-2 gap-3">
              {/* Filter: Período */}
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-[#5A6270] uppercase ml-2 tracking-widest font-display opacity-80">Período</label>
                <select
                  value={monthFilter}
              ...
              {/* Filter: Status */}
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-[#5A6270] uppercase ml-2 tracking-widest font-display opacity-80">Status</label>
                <select
                  value={statusFilter}
                  onChange={(e) => setStatusFilter(e.target.value)}
                  className="w-full bg-[#F4F5F7] border border-transparent rounded-2xl px-5 py-3.5 text-xs font-bold text-app-text outline-none focus:border-brand-primary transition-all appearance-none cursor-pointer"
            ...
            <form onSubmit={handleSearch} className="relative group pt-2">
              <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-[#5A6270] opacity-30" size={20} />
              <input
                type="text"
                className="w-full bg-white border border-[#E5E7EB] rounded-2xl py-4.5 pl-14 pr-5 outline-none focus:border-brand-primary focus:ring-4 focus:ring-brand-primary/5 transition-all text-sm font-semibold text-app-text placeholder:text-[#9CA3AF]"
          ...
          <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} className="space-y-6">
            <div className={`p-8 rounded-[40px] border-2 ${isReleased(result.data_liberacao_ecoordin) ? 'bg-white border-success/20' : 'bg-white border-danger/20'} shadow-floating relative overflow-hidden`}>
              <div className="relative z-10 flex flex-col gap-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-2 gap-3">
                  ...
                  <div className="col-span-full md:col-span-1 lg:col-span-1 xl:col-span-full">
          ...
          (
            <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              ...
              {filteredList.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-1 gap-4 pb-12">
